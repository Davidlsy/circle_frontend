...
async function sendSticker(sticker) {
  if (sending.value || !selectedConvId.value) return
  sending.value = true
  try {
    const tempMsg = {
      id: Date.now(),
    ...
    await sendMessage(selectedConvId.value, { content: sticker.url, message_type: 'sticker' })
    await markRead(selectedConvId.value).catch(() => {})
    await fetchMessages(selectedConvId.value)
  } catch (e) {
